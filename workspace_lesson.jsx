...
function LessonWorkspace({ scenario, query, onHome, onSwitch, fromIntent, resume, loggedIn, nav }) {
  ...
  const greet = <span>我来帮你写<b style={{ color: "var(--brand-deep)" }}>备课文档</b>——教案（教学设计、讲义、学历案等）或学案（导学案、学习任务单等）都行。先在右上选好教材与类型，然后告诉我课题。</span>;
  ...
  const doneNote = (d) => (
    <span>《<b>{d.topic}</b>》的{d.docType || "教学设计"}已经成稿——按{d.edition}{d.grade}{d.subject}写的，结构对齐<b style={{ color: "var(--auth-ink)" }}>课程标准与权威范例</b>。右侧任何一段都可以直接点击修改，也可以继续吩咐我调整。</span>
  );
  ...
  return (
    <WorkspaceShell scenario={scenario} onHome={onHome} onSwitch={onSwitch} nav={nav} headerRecognizing={headerRecognizing} mobilePanelLabel="教案" mobilePanelIcon="lesson" openSheetKey={doc ? doc.topic : null}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", background: "var(--canvas)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: doc ? "10px 16px" : "9px 16px", borderBottom: "1px solid var(--line)", background: "var(--surface)", flexShrink: 0 }}>
          {doc ? (
            /* 编辑阶段：极简头——返回 + 当前文档 + 下载 */
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1 }} />
              <Btn size="sm" kind="soft" icon="download" onClick={exportDoc}>下载</Btn>
            </div>
          ) : (
          <React.Fragment>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <div style={{ display: "flex", gap: 2, padding: 2, borderRadius: 9, background: "var(--surface-2)", border: "1px solid var(--line)" }}>
              {LESSON_CATEGORIES.map((c) => {
                const on = docCat === c.key;
                return <button key={c.key} onClick={() => chooseCat(c.key)} style={{ padding: "4px 11px", borderRadius: 7, border: "none", background: on ? "var(--surface)" : "transparent", color: on ? "var(--brand-deep)" : "var(--ink-3)", fontSize: 12, fontWeight: 800, cursor: on ? "default" : "pointer", fontFamily: "var(--font-zh)", boxShadow: on ? "0 1px 3px rgba(0,0,0,.06)" : "none" }}>{c.key}</button>;
              })}
            </div>
            <div style={{ display: "flex", gap: 4 }}>
              {(LESSON_CATEGORIES.find((c) => c.key === docCat) || LESSON_CATEGORIES[0]).types.map((t) => {
                const on = docType === t;
                return <button key={t} onClick={() => chooseType(t)} style={{ padding: "4px 10px", borderRadius: 7, border: on ? "1px solid var(--brand-soft-border)" : "1px solid transparent", background: on ? "var(--brand-soft)" : "transparent", color: on ? "var(--brand-deep)" : "var(--ink-3)", fontSize: 11.5, fontWeight: on ? 700 : 600, cursor: on ? "default" : "pointer", fontFamily: "var(--font-zh)" }}>{t}</button>;
              })}
            </div>
          </div>
          </React.Fragment>
        ...
        <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, zIndex: 85, width: mobile ? "100%" : "min(560px, 94vw)", background: "var(--canvas)", boxShadow: "-20px 0 60px -30px rgba(0,0,0,.5)", transform: pickTextbook ? "translateX(0)" : "translateX(102%)", transition: "transform .26s cubic-bezier(.22,1,.36,1)", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ flex: 1, minHeight: 0, overflowY: "auto" }}>
            {pickTextbook && <LessonTextbookPicker current={textbook} onApply={(tb) => { setTextbook(tb); setPickTextbook(false); }} onWrite={(tb, topic) => { setTextbook(tb); setPickTextbook(false); send(`${tb.edition}${lzGrade(tb)}${tb.subject}《${topic}》${docType}`); }} docType={docType} mobile={mobile} />}
          </div>
        </div>
      </div>
    </WorkspaceShell>
...
function LessonStartPage({ textbook, docType, loggedIn, savedDocs, onWrite, onPickTextbook, onExample, onOpenSaved, mobile }) {
  ...
  return (
    <div className="home-fade" style={{ width: "min(560px,100%)", margin: "0 auto", paddingBottom: 8 }}>
      {/* Hero —— 引导用自然语言，对话优先 */}
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        <div style={{ display: "inline-flex", marginBottom: 13 }}><ScenarioGlyph icon="lesson" hue={320} size={52} active /></div>
        <h2 style={{ fontSize: 19, fontWeight: 800, color: "var(--ink)", margin: "0 0 6px" }}>写一份{docType}</h2>
        <p style={{ fontSize: 13, color: "var(--ink-2)", margin: 0, lineHeight: 1.6 }}>在左侧对话框用一句话告诉我需求就行，比如：</p>
      </div>

      {/* 示例问法 —— 点一下就替你说给左侧 */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginBottom: 22 }}>
        {examples.map((ex, i) => (
          <button key={i} onClick={() => onExample(ex)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 13px", borderRadius: 999, border: "1px solid var(--line)", background: "var(--surface)", color: "var(--ink-2)", fontSize: 12.5, fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-zh)", transition: "all .15s" }}
            ...
            <Icon name="spark" size={13} /> {ex}
          </button>
        ))}
      </div>

      {/* 本册目录 —— 近顶部、可折叠，需要时再展开自己选 */}
      <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden", marginBottom: 16 }}>
        ...
        {catOpen && (
          <div className="zx-pop" style={{ borderTop: "1px solid var(--line)" }}>
            ...
            <button onClick={onPickTextbook} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "11px", border: "none", borderTop: "1px solid var(--line)", background: "var(--surface-2)", color: "var(--ink-3)", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-zh)" }}
              onMouseEnter={(e) => { e.currentTarget.style.color = "var(--brand-deep)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "var(--ink-3)"; }}>
              <Icon name="refresh" size={13} /> 换一本教材
            </button>
          </div>
        )}
      </div>
